...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    metrics.circuit_open_count = sum(
        1 for b in gateway.breakers.values() for t in b.transition_log if t.get("to") == "open"
    )
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            passed = result.fallback_success_rate >= 0.9 and result.successful_requests > 0
        ...
        if result.recovery_time_ms is not None:
            all_recovery_times.append(result.recovery_time_ms)

    if all_recovery_times:
        combined.recovery_time_ms = sum(all_recovery_times) / len(all_recovery_times)
